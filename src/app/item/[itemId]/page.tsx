...
export default function ItemDetailPage() {
  ...
  const basePrice = item.isDiscount ? Math.floor(item.price * (1 - item.discountRate / 100)) : item.price;
          ...
          <div className="aspect-square w-full overflow-hidden rounded-sm bg-stone-100 relative border border-stone-200">
            ...
            {item.itemSellStatus === "SOLD_OUT" && (
              <div className="absolute inset-0 bg-black/50 flex items-center justify-center">
                <span className="text-white text-2xl font-bold border-2 border-white px-6 py-2">
                  SOLD OUT
                </span>
              </div>
            )}
        ...
        <div className="flex flex-col h-full">
          <div className="border-b border-stone-200 pb-6 mb-6">
            ...
            <div className="flex items-end gap-3">
              ...
              <span className="text-2xl font-bold text-primary">{basePrice.toLocaleString()}원</span>
              {item.isDiscount && (
                <span className="text-lg text-red-600 font-medium">{item.discountRate}% OFF</span>
              )}
              {item.isPayback && (
            ...
            <div className="space-y-4 pt-4">
              ...
              <div className="bg-stone-50 p-4 rounded text-sm text-stone-600 space-y-1">
                <div className="flex justify-between">
                  <span>배송비</span>
                  <span>
                    {item.deliveryFee === 0 ? "무료배송" : `${item.deliveryFee.toLocaleString()}원`}
                  </span>
                </div>
                <div className="flex justify-between">
                  <span>원산지</span>
          ...
          <div className="mt-8 border-t border-stone-200 pt-6">
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="text-right">
                <span className="text-sm text-stone-500 mr-2">총 수량 {quantity}개</span>
