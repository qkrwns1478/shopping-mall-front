...
export default function CartPage() {
  ...
  const selectedCartItems = cartItems.filter(
    (item) => item.cartItemId !== undefined && selectedIds.has(item.cartItemId)
  );

  const productAmount = selectedCartItems.reduce((acc, item) => {
    const itemPrice = item.isDiscount ? Math.floor(item.price * (1 - item.discountRate / 100)) : item.price;
  ...
  if (cartItems.length === 0) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        ...
        <Link
          href="/"
          className="px-6 py-3 bg-primary text-white rounded font-bold hover:bg-primary-dark transition"
        >
          쇼핑하러 가기
        </Link>
      </div>
        ...
        <div className="flex-1 space-y-4">
          ...
          {cartItems.map((item) => {
            ...
            return (
              <div
                key={item.cartItemId}
                className={`flex gap-4 p-4 bg-white border rounded-lg shadow-sm transition-colors ${
                  isChecked ? "border-primary/50 ring-1 ring-primary/20" : "border-gray-200"
                }`}
              >
                {/* 개별 체크박스 */}
                <div className="flex items-center">
                    ...
                    <div className="text-sm text-gray-500 mt-1 space-y-0.5">
                      {item.optionName && (
                        <p>
                          옵션: {item.optionName}{" "}
                          {item.optionPrice > 0 &&
                            `(+${item.optionPrice.toLocaleString()}원)`}
                        </p>
                      )}
